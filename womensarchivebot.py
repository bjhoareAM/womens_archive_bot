...
if '--help' in arg_vals or '-H' in arg_vals:  # provide help information according to GNU standards
    ...
    print('Report bugs to:bjhoare@aucklandmuseum.com.')
    sys.exit()
...
if '--log' in opts:  # set output to specified log file or path including file name
    ...
    log_object = open(log_path, 'wt',
                      encoding='utf-8')  # direct output sent to log_object to log file instead of sys.stdout
...
if '--apisleep' in opts:  # delay between API POSTs. Used by newbies to slow writes to within limits.
    api_sleep = int(
        args[opts.index('--apisleep')])  # Number of seconds between API calls. Numeric only, do not include "s"
if '-A' in opts:
...
def findPropertyUuid(propertyId, columns):
    ...
    for column in columns:
        if not ('suppressOutput' in column):
            ...
            valueString = column['propertyUrl'].partition('prop/')[
                2]  # This will pick up all kinds of properties, but only p: properties will have PID directly after 'prop/'
            if propertyId == valueString:
                nUuidColumns += 1
                temp = column['valueUrl'].partition('-{')[2]
                statementUuidColumn = temp.partition('}')[
                    0]  # in the event of two columns with the same property ID, the last one is used
    ...
    for column in columns:
        if not ('suppressOutput' in column):
            ...
            if ('prov:wasDerivedFrom' in column['propertyUrl']) and (statementUuidColumn in column['aboutUrl']):
                                ...
                                for testColumn in columns:
                                    try:
                                        if propColumn['titles'] in testColumn['aboutUrl']:
                                            ...
                                            elif 'geoLatitude' in testColumn[
                                                'propertyUrl']:  # value is a globe coordinate value
                                                refEntityOrLiteral.append('value')
                ...
                referenceList.append({'refHashColumn': refHashColumn, 'refPropList': refPropList,
                                      'refValueColumnList': refValueColumnList,
                                      'refEntityOrLiteral': refEntityOrLiteral, 'refTypeList': refTypeList,
                                      'refValueTypeList': refValueTypeList, 'refLangList': refLangList})
...
def findQualifiersForProperty(statementUuidColumn, columns):
    ...
    qualifierDictionary = {'qualPropList': qualPropList, 'qualValueColumnList': qualValueColumnList,
                           "qualEntityOrLiteral": qualEntityOrLiteral, 'qualTypeList': qualTypeList,
                           'qualValueTypeList': qualValueTypeList, 'qualLangList': qualLangList}
...
def generateSnaks(snakDictionary, require_references, refValue, refPropNumber, refPropList, refValueColumnList,
                  refValueTypeList, refTypeList, refEntityOrLiteral):
        ...
        for refPropNumber in range(0, len(refPropList)):
            if refEntityOrLiteral[refPropNumber] == 'value':
                ...
                if rowData[refValueColumnList[refPropNumber] + '_nodeId'] == '':
                    ...
                    if refTypeList[refPropNumber] == 'time':
                        refValue = {'timeValue': rowData[refValueColumnList[refPropNumber] + '_val'],
                                    'timePrecision': rowData[refValueColumnList[refPropNumber] + '_prec']}
                    elif refTypeList[refPropNumber] == 'quantity':
                        refValue = {'amount': rowData[refValueColumnList[refPropNumber] + '_val'],
                                    'unit': rowData[refValueColumnList[refPropNumber] + '_unit']}
                    elif refTypeList[refPropNumber] == 'globe-coordinate':
                        refValue = {'latitude': rowData[refValueColumnList[refPropNumber] + '_val'],
                                    'longitude': rowData[refValueColumnList[refPropNumber] + '_long'],
                                    'precision': rowData[refValueColumnList[refPropNumber] + '_prec']}
                ...
                if rowData[refValueColumnList[refPropNumber]] == '':
                    ...
                    refValue = {'text': rowData[refValueColumnList[refPropNumber]],
                                'language': refLangList[refPropNumber]}
            else:
                refValue = rowData[refValueColumnList[refPropNumber]]
            snakDictionary = generateSnaks(snakDictionary, require_references, refValue, refPropNumber, refPropList,
                                           refValueColumnList, refValueTypeList, refTypeList, refEntityOrLiteral)
        ...
        if qualEntityOrLiteral[qualPropNumber] == 'value':
            ...
            if rowData[qualValueColumnList[qualPropNumber] + '_nodeId'] == '':
                ...
                if qualTypeList[qualPropNumber] == 'time':
                    qualValue = {'timeValue': rowData[qualValueColumnList[qualPropNumber] + '_val'],
                                 'timePrecision': rowData[qualValueColumnList[qualPropNumber] + '_prec']}
                elif qualTypeList[refPropNumber] == 'quantity':
                    qualValue = {'amount': rowData[qualValueColumnList[qualPropNumber] + '_val'],
                                 'unit': rowData[qualValueColumnList[qualPropNumber] + '_unit']}
                elif qualTypeList[refPropNumber] == 'globe-coordinate':
                    qualValue = {'latitude': rowData[qualValueColumnList[qualPropNumber] + '_val'],
                                 'longitude': rowData[qualValueColumnList[qualPropNumber] + '_long'],
                                 'precision': rowData[qualValueColumnList[qualPropNumber] + '_prec']}
                else:
                    pass
            ...
            if rowData[qualValueColumnList[qualPropNumber]] == '':
                ...
                qualValue = {'text': rowData[qualValueColumnList[qualPropNumber]],
                             'language': qualLangList[qualPropNumber]}
        else:
            qualValue = rowData[qualValueColumnList[qualPropNumber]]
        snakDictionary = generateSnaks(snakDictionary, require_qualifiers, qualValue, qualPropNumber, qualPropList,
                                       qualValueColumnList, qualValueTypeList, qualTypeList, qualEntityOrLiteral)
    return snakDictionary
...
for table in tables:  # The script can handle multiple tables
        ...
        if 'suppressOutput' in column:
            ...
            if column['name'] == 'alias':
                ...
                for entityIndex in range(0, len(tableData)):
                    personAliasList = []
                    if tableData[entityIndex][
                        subjectWikidataIdColumnHeader] != '':  # don't look for the label at Wikidata if the item doesn't yet exist
        ...
        # handle all other non-suppressed columns.
            ...
            # Note: if descriptions exist for a language, they will be overwritten
                ...
                if allow_label_description_changes:
                    ...
                    tempLabels = []
                    descriptionsAtWikidata = searchLabelsDescriptionsAtWikidata(qIds, 'description',
                                                                                descriptionLanguage)
                    for entityIndex in range(0, len(tableData)):
                        found = False
                        if tableData[entityIndex][
            ...
            # find columns that contain properties with entity values, literal values that are URLs, or value node values
                ...
                if 'prop/statement/' in column['propertyUrl']:
                    ...
                    if 'prop/statement/value/' in column[
                        'propertyUrl']:  # value is a value node (e.g. date or geo coordinates)
                        found = True
                        propColumnHeader = column['titles'].partition('_nodeId')[
                            0]  # save only the root of the column name for value nodes
            ...
            # remaining columns should have properties with literal values
                ...
                if 'prop/statement/' in column['propertyUrl']:
                    ...
                    if 'lang' in column:
                        if not terse:
                            print('Property column: ', propColumnHeader, ', Property ID: ', propertyId,
                                  ' Value type: monolingualtext  Language: ', column['lang'])
        ...
        for propertyNumber in range(0, len(propertiesColumnList)):
            ...
            if len(propertiesQualifiersList[propertyNumber]) != 0:
                for qualPropNumber in range(0, len(propertiesQualifiersList[propertyNumber]['qualPropList'])):
                    if propertiesQualifiersList[propertyNumber]['qualTypeList'][qualPropNumber] == 'time':
                        ...
                        dateColumnNameList.append(
                            propertiesQualifiersList[propertyNumber]['qualValueColumnList'][qualPropNumber])
                    if propertiesQualifiersList[propertyNumber]['qualEntityOrLiteral'][qualPropNumber] == 'value':
                        valueColumnNameList.append(
                            propertiesQualifiersList[propertyNumber]['qualValueColumnList'][qualPropNumber])
            ...
            if len(propertiesReferencesList[propertyNumber]) != 0:
                for referenceNumber in range(0, len(propertiesReferencesList[propertyNumber])):
                    for refPropNumber in range(0, len(
                            propertiesReferencesList[propertyNumber][referenceNumber]['refPropList'])):
                        if propertiesReferencesList[propertyNumber][referenceNumber]['refTypeList'][
                            refPropNumber] == 'time':
                            ...
                            dateColumnNameList.append(
                                propertiesReferencesList[propertyNumber][referenceNumber]['refValueColumnList'][
                                    refPropNumber])
                        if propertiesReferencesList[propertyNumber][referenceNumber]['refEntityOrLiteral'][
                            refPropNumber] == 'value':
                            valueColumnNameList.append(
                                propertiesReferencesList[propertyNumber][referenceNumber]['refValueColumnList'][
                                    refPropNumber])
    ...
    for rowNumber in range(0, len(tableData)):
        ...
        if len(labelColumnList) > 0:  # skip printing a label if there aren't any
            status_message += '  Label: ' + tableData[rowNumber][
                labelColumnList[0]]  # include the first label available
        ...
        if new_item:
            ...
            for language in language_structure:
                ...
                stripped = False
                if tableData[rowNumber][language['label_column']].strip() != tableData[rowNumber][
                    language['label_column']]:
                    tableData[rowNumber][language['label_column']] = tableData[rowNumber][
                        language['label_column']].strip()
                    stripped = True
                if tableData[rowNumber][language['description_column']].strip() != tableData[rowNumber][
                    language['description_column']]:
                    tableData[rowNumber][language['description_column']] = tableData[rowNumber][
                        language['description_column']].strip()
                    stripped = True
                ...
                if language['label_column'] != '' and language['description_column'] != '':
                    ...
                    if tableData[rowNumber][language['label_column']] != '' and tableData[rowNumber][
                        language['description_column']] != '':
                        has_some_value = True
                        if duplicate_check:
                            graph_pattern = '  ?entity rdfs:label ' + safe_quotes(
                                tableData[rowNumber][language['label_column']]) + '@' + language['language'] + '''.
        ?entity schema:description ''' + safe_quotes(tableData[rowNumber][language['description_column']]) + '@' + \
                                            language['language'] + '.'
                            ...
                            if exists:
                                ...
                                error_log += 'Duplicate label/description. Row: ' + str(rowNumber) + ', language: "' + \
                                             language['language'] + '", label: "' + tableData[rowNumber][
                                                 language['label_column']] + '", description: "' + tableData[rowNumber][
                                                 language['description_column']] + '"\n'

                    # Case where the label has a value but description does not
                    elif tableData[rowNumber][language['label_column']] != '' and tableData[rowNumber][
                        language['description_column']] == '':
                        has_some_value = True
                        if duplicate_check:
                            exists = check_for_only_label(tableData[rowNumber][language['label_column']],
                                                          language['language'])
                            if exists:
                                combination_exists = True
                                error_log += 'Duplicate label only. Row: ' + str(rowNumber) + ', language: "' + \
                                             language['language'] + '", label: "' + tableData[rowNumber][
                                                 language['label_column']] + '"\n'
                    ...
                    # Case where the description has a value but label does not
                        ...
                        has_some_value = True
                        if duplicate_check:
                            exists = check_for_only_description(tableData[rowNumber][language['description_column']],
                                                                language['language'])
                            if exists:
                                combination_exists = True
                                error_log += 'Duplicate description only. Row: ' + str(rowNumber) + ', language: "' + \
                                             language['language'] + '", label: "' + tableData[rowNumber][
                                                 language['description_column']] + '"\n'
            ...
            if propertiesTypeList[propertyNumber] == 'commonsMedia':
                ...
                if valueString != '' and valueString[:2] != '_:':
                    if not commons_prefix in valueString:  # convert from unencoded filename if the URL prefix isn't there
                        tableData[rowNumber][propertiesColumnList[propertyNumber]] = filename_to_commons_url(
                            valueString)  # replace the tabled value
                        cells_converted = True

        # Convert any blank node abbreviations into Turtle bnode strings with UUID identifiers
        for propertyNumber in range(len(propertiesColumnList)):
            if propertiesEntityOrLiteral[propertyNumber] == 'value':
                changed, tableData[rowNumber][propertiesColumnList[propertyNumber] + '_val'] = generate_uuid_bnode(
                    tableData[rowNumber][propertiesColumnList[propertyNumber] + '_val'])
            else:
                changed, tableData[rowNumber][propertiesColumnList[propertyNumber]] = generate_uuid_bnode(
                    tableData[rowNumber][propertiesColumnList[propertyNumber]])
        ...
        for propertyNumber in range(len(propertiesColumnList)):
            if propertiesEntityOrLiteral[propertyNumber] == 'value':
                string_value, error = remove_leading_trailing_whitespace(
                    tableData[rowNumber][propertiesColumnList[propertyNumber] + '_val'])
                if error:
                ...
                if newItem:
                    ...
                    if allow_label_description_changes:
                        ...
                        if valueString != existingDescriptions[languageNumber][rowNumber]:
                            # if they are different check to make sure the table value isn't empty
                            if valueString != '':
                                print('Changing description ', existingDescriptions[languageNumber][rowNumber], ' to ',
                                      valueString, file=log_object)
                                # add the description in the table for that language to the description dictionary
                                descriptionDict[descriptionLanguageList[languageNumber]] = {
                                    'language': descriptionLanguageList[languageNumber],
                                    'value': valueString
                                ...
                                'mainsnak': {
                                    ...
                                    'datavalue': {
                                        'value': {
                                            ...
                                            'before': 0,
                                            'after': 0,
                                            'precision': tableData[rowNumber][
                                                propertiesColumnList[propertyNumber] + '_prec'],
                                            'calendarmodel': 'http://www.wikidata.org/entity/' + calendar_model
                                        },
                                        'type': 'time'
                                    },
                            ...
                            snakDict = {
                                'mainsnak': {
                                    ...
                                    'datavalue': {
                                        'value': {
                                            'amount': valueString,
                                            # a string for a decimal number; must have leading + or -
                                            'unit': DOMAIN_NAME + '/entity/' + tableData[rowNumber][
                                                propertiesColumnList[propertyNumber] + '_unit']  # IRI as a string
                                        },
                                        'type': 'quantity',
                                    },
                                    'datatype': 'quantity'
                                ...
                                'mainsnak': {
                                    ...
                                    'datavalue': {
                                        'value': {
                                            ...
                                            'longitude': float(
                                                tableData[rowNumber][propertiesColumnList[propertyNumber] + '_long']),
                                            # longitude; decimal number
                                            'precision': float(
                                                tableData[rowNumber][propertiesColumnList[propertyNumber] + '_prec']),
                                            # precision; decimal number
                                            'globe': 'http://www.wikidata.org/entity/' + globe_value
        ...
        if parameterDictionary['data'] == '{}':
            ...
            if 'error' in responseData:
                error_log += 'Error message from API in row ' + str(rowNumber) + ': ' + responseData['error'][
                    'info'] + '\n'
            ...
            for statementIndex in range(0, len(propertiesIdList)):
                ...
                if propertiesEntityOrLiteral[statementIndex] == 'value':
                    if tableData[rowNumber][propertiesColumnList[statementIndex] + '_nodeId'] == '':
                        ...
                        if len(tableData[rowNumber][propertiesColumnList[statementIndex] + '_val']) >= 2 and \
                                tableData[rowNumber][propertiesColumnList[statementIndex] + '_val'][:2] == '_:':
                            value = True
                        else:
                            value = False
                    else:
                ...
                if tableData[rowNumber][propertiesUuidColumnList[statementIndex]] == '' and value:
                    ...
                    for statement in responseData['entity'][CLAIM_KEY][propertiesIdList[statementIndex]]:
                        ...
                        if statement['mainsnak']['snaktype'] == 'novalue':
                            ...
                            value_value_bnode = propertiesEntityOrLiteral[statementIndex] == 'value' and len(
                                tableData[rowNumber][propertiesColumnList[statementIndex] + '_val']) >= 2 and \
                                                tableData[rowNumber][propertiesColumnList[statementIndex] + '_val'][
                                                :2] == '_:'
                            plain_value_bnode = propertiesEntityOrLiteral[statementIndex] != 'value' and len(
                                tableData[rowNumber][propertiesColumnList[statementIndex]]) >= 2 and \
                                                tableData[rowNumber][propertiesColumnList[statementIndex]][:2] == '_:'
                            ...
                            if propertiesEntityOrLiteral[statementIndex] == 'literal':
                                ...
                                if propertiesTypeList[statementIndex] == 'commonsMedia':
                                    statementFound = commons_url_to_filename(
                                        tableData[rowNumber][propertiesColumnList[statementIndex]]) == \
                                                     statement['mainsnak']['datavalue']['value']
                                else:
                                    statementFound = tableData[rowNumber][propertiesColumnList[statementIndex]] == \
                                                     statement['mainsnak']['datavalue']['value']
                                ...
                                statementFound = tableData[rowNumber][propertiesColumnList[statementIndex]] == \
                                                 statement['mainsnak']['datavalue']['value']['text'] and \
                                                 propertiesLangList[statementIndex] == \
                                                 statement['mainsnak']['datavalue']['value']['language']
                                ...
                                if propertiesTypeList[statementIndex] == 'time':
                                    ...
                                    if tableData[rowNumber][propertiesColumnList[statementIndex] + '_val'][0] == '-':
                                        ...
                                        statementFound = tableData[rowNumber][
                                                             propertiesColumnList[statementIndex] + '_val'] == \
                        ...
                        if statementFound:
                            ...
                            if count > 1:
                                ...
                                dup_message = 'Warning: duplicate statement ' + tableData[rowNumber][
                                    subjectWikidataIdColumnHeader] + ' ' + propertiesIdList[statementIndex] + ' '
                                if propertiesEntityOrLiteral[statementIndex] == 'value':
                            ...
                            tableData[rowNumber][propertiesUuidColumnList[statementIndex]] = statement['id'].split('$')[
                                1]  # just keep the UUID part after the dollar sign
                            ...
                            if 'references' in statement:  # skip reference checking if the item doesn't have any references
                                ...
                                for tableReference in referencesForStatement:  # loop will not be executed when length of referenceForStatement = 0 (no references tracked for this property)
                                    ...
                                    for responseReference in statement['references']:  # "outer loop"
                                        ...
                                        referenceMatch = True
                                        refWithValues = False
                                        for referencePropertyIndex in range(0, len(tableReference[
                                                                                       'refPropList'])):  # "inner loop" to check each property in the reference
                                            try:
                                                # First try to see if the values in the response JSON for the property match
                                                if tableReference['refEntityOrLiteral'][
                                                    referencePropertyIndex] == 'value':
                                                    # Skip checking the property if it doesn't have a value
                                                    if tableData[rowNumber][tableReference['refValueColumnList'][
                                                                                referencePropertyIndex] + '_val'] != '':
                                                        ...
                                                        if tableReference['refTypeList'][
                                                            referencePropertyIndex] == 'time':
                                                            # need to handle negative dates (BCE)
                                                            if tableData[rowNumber][
                                                                tableReference['refValueColumnList'][
                                                                    referencePropertyIndex] + '_val'][0] == '-':
                                                                # make comparison with the leading minus present
                                                                if responseReference['snaks'][
                                                                    tableReference['refPropList'][
                                                                        referencePropertyIndex]][0]['datavalue'][
                                                                    'value']['time'] != tableData[rowNumber][
                                                                    tableReference['refValueColumnList'][
                                                                        referencePropertyIndex] + '_val']:
                                                                ...
                                                                if responseReference['snaks'][
                                                                    ...
                                                                    referenceMatch = False
                                                                    break  # kill the inner loop because this value doesn't match
                                                        elif tableReference['refTypeList'][
                                                            referencePropertyIndex] == 'quantity':
                                                            # need to handle negative quantities
                                                            if tableData[rowNumber][
                                                                tableReference['refValueColumnList'][
                                                                    referencePropertyIndex] + '_val'][0] == '-':
                                                                ...
                                                                if responseReference['snaks'][
                                                                    tableReference['refPropList'][
                                                                        referencePropertyIndex]][0]['datavalue'][
                                                                    'value']['amount'] != tableData[rowNumber][
                                                                    tableReference['refValueColumnList'][
                                                                        referencePropertyIndex] + '_val']:
                                                                    referenceMatch = False
                                                                    break  # kill the inner loop because this value doesn't match
                                                            ...
                                                            referencePropertyIndex] == 'globe-coordinate':
                                                            if responseReference['snaks'][
                                                                tableReference['refPropList'][referencePropertyIndex]][
                                                                0]['datavalue']['value']['latitude'] != float(
                                                                    tableData[rowNumber][
                                                                        tableReference['refValueColumnList'][
                                                                            referencePropertyIndex] + '_val']):
                                                                referenceMatch = False
                                                    ...
                                                    if tableData[rowNumber][tableReference['refValueColumnList'][
                                                        ...
                                                        refWithValues = True  # at least one reference had a value
                                                        if tableReference['refEntityOrLiteral'][
                                                            referencePropertyIndex] == 'monolingualtext':
                                                            # The monolingual text (language-tagged literals) have an additional layer "text" within the value
                                                            if responseReference['snaks'][
                                                                tableReference['refPropList'][referencePropertyIndex]][
                                                                0]['datavalue']['value']['text'] != \
                                                                    tableData[rowNumber][
                                                                        tableReference['refValueColumnList'][
                                                                            referencePropertyIndex]] and \
                                                                    responseReference['snaks'][
                                                                        tableReference['refPropList'][
                                                                            referencePropertyIndex]][0]['datavalue'][
                                                                        'value']['language'] != \
                                                                    tableReference['refLangList'][
                                                                        referencePropertyIndex]:
                                                                referenceMatch = False
                                                                break  # kill the inner loop because this value doesn't match
                                                            ...
                                                            if responseReference['snaks'][
                                                                tableReference['refPropList'][referencePropertyIndex]][
                                                                0]['datavalue']['value'] != tableData[rowNumber][
                                                                tableReference['refValueColumnList'][
                                                                    referencePropertyIndex]]:
                                                                referenceMatch = False
                                                                break  # kill the inner loop because this value doesn't match
                                        ...
                                        if refWithValues:
                                            tableData[rowNumber][tableReference['refHashColumn']] = responseReference[
                                                'hash']
                        ...
                        if tableData[rowNumber][reference['refHashColumn']] == '':  # process only new references
                            ...
                            if referencesDict == {}:  # Check for the case where no references were specified for this record
                                ...
                                parameterDictionary = {
                                    ...
                                    'statement': tableData[rowNumber][subjectWikidataIdColumnHeader] + "$" +
                                                 tableData[rowNumber][statementUuidColumn],
                                    'format': 'json',
                                    'token': csrfToken,
